...
TaggingInfo identifier(const TaggedType& tagged_type, const Module& current_module, const Asn1Tree& tree)
{
    ...
    if (is_explicit || !child_tag.outer_tag.class_.empty())
    {
        return TaggingInfo{Identifier{to_string(tagged_type.tag.class_, true), tagged_type.tag.tag_number},
                           child_tag.inner_tag, false};
    }
    else
    {
        return TaggingInfo{{},
                           "IId<" + to_string(tagged_type.tag.class_, true) + ", " +
