...
CodeBlock visit_all_types(const Asn1Tree& tree, const Module& module, const Type& type, const std::string& context_namespace, Function&& func)
{
    CodeBlock block;
    ...
    if (is_sequence(type))
    {
        ...
        for (const ComponentType& component : sequence.components)
        {
            ...
            block.add_block(visit_all_types(tree, module, component.named_type.type, name, std::forward<Function>(func)));
        }
    }
    else if (is_set(type))
    {
    ...
    else if (is_sequence_of(type))
    {
        ...
        if (sequence.has_name)
        {
            ...
            block.add_block(visit_all_types(tree, module, sequence.named_type->type, name, std::forward<Function>(func)));
        }
    }
    ...
    else if (is_prefixed(type))
    {
        ...
        block.add_block(visit_all_types(tree, module, prefixed.tagged_type->type, context_namespace, std::forward<Function>(func)));
    }
...
CodeBlock visit_all_types(const Asn1Tree& tree, const Module& module, const Assignment& assignment, Function&& func)
{
    if (absl::holds_alternative<TypeAssignment>(assignment.specific))
    {
        ...
        return visit_all_types(tree, module, type_assignment.type, "fast_ber::" + module.module_reference + "::" +assignment.name, std::forward<Function>(func));
    }
    return {};
}
