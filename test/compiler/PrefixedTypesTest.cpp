...
TEST_CASE("Prefixed Types: Encoding, matching 8.14 of X.690")
{
    ...
    res = fast_ber::encode(absl::MakeSpan(buffer.data(), buffer.size()), type1);
    REQUIRE(res.success);
    ...
    res = fast_ber::encode(absl::MakeSpan(buffer.data(), buffer.size()), type4);
    REQUIRE(res.success);
    REQUIRE(absl::MakeSpan(buffer.data(), 9) ==
            std::array<uint8_t, 9>{0x67, 0x07, 0x43, 0x05, 0x4A, 0x6F, 0x6E, 0x65, 0x73});

    res = fast_ber::encode(absl::MakeSpan(buffer.data(), buffer.size()), type5);
    REQUIRE(res.success);
